...
<body id="page-top">
	...
	<div id="content-wrapper" class="d-flex flex-column">
		...
		<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
			<h1 class="h3 mb-0 text-gray-800">사용자 설정  >  회원 정보 수정</h1>
			<!-- Topbar Navbar -->
					...
					<div class="wrap-login100">
						<form class="login100-form validate-form">
							...
							<div class="wrap-input100 validate-input" data-validate = "Enter userId">
								<input class="input100" type="text" id="USERID" name="USERID" value="${UserLogin.USER_ID}" disabled="disabled">
							</div>
							<div class="wrap-input100 validate-input" data-validate = "Enter username">
								<input class="input100" type="text" id="USERNM" name="USERNM" placeholder="이름 ( ex> 윤준,yoon jun )" value="${UserLogin.USER_NM}">
							</div>
							...
							<div class="wrap-input100 validate-input" data-validate = "Enter MOBILE">
								<input class="input100" type="text"  id="MOBILE" name="MOBILE" placeholder="핸드폰 번호( ex>010-0000-0000 )" value="${UserLogin.TEL}">
							</div>
							<div class="wrap-input100 validate-input" data-validate="Enter EMAIL">
								<input class="input100" type="text" id="EMAIL" name="EMAIL" placeholder="이메일 양식( ex>y@naver.com )" value="${UserLogin.EMAIL}">
							</div>
							...
							<div class="wrap-input100 validate-input">
								<select id="TEAM_CD"  name="TEAM_CD">
									...
									<option value=1 <c:if test="${UserLogin.TEAM_CD eq 1}">selected </c:if>>개발</option>
									<option value=2 <c:if test="${UserLogin.TEAM_CD eq 2}">selected </c:if>>영업</option>
									<option value=3 <c:if test="${UserLogin.TEAM_CD eq 3}">selected </c:if>>인사</option>
								</select>
								
								<select id="POWER" name="POWER">
									<option value=0>직급 선택</option>
									<option value=1 <c:if test="${UserLogin.POWER eq 1}">selected </c:if>>사원</option>
									<option value=2 <c:if test="${UserLogin.POWER eq 2}">selected </c:if>>대리</option>
									<option value=3 <c:if test="${UserLogin.POWER eq 3}">selected </c:if>>과장</option>
									<option value=4 <c:if test="${UserLogin.POWER eq 4}">selected </c:if>>부장</option>
								</select>
							</div>
							<br>
							<div class="container-login100-form-btn">
								<button type="button" class="login100-form-btn" onclick="fn_update();">
									수정
								</button>
							</div>
						</form>
	...
	function fn_update(){
		...
		var USERUPDATE={"USERID" : USERID , "USERNM" : USERNM , "PASSWD" : PASSWD , "TEL" : TEL , "EMAIL" : EMAIL , "TEAMCD" : TEAMCD ,"POWER" : POWER };
		
		$.ajax({

			url: "/set/userupdate.do",
		    type: "post",
			dataType: "json",
			data: USERUPDATE,
			success: function(data){

				if(data.userupdate == '1' | data.userupdate ==1 ){

					alert("정상적으로 수정 되었습니다");
					window.location.href = "/set/userlist.do";
				}else{
					alert("오류가 발생하여 정상적으로 수정 되지 않았습니다")
				}
